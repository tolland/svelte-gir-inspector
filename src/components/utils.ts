...
const searchInElement = (searchTerm: string, element: any, type: string, currentPath: string) => {
    if (element.name.toLowerCase().includes(searchTerm.toLowerCase())) {
        results.push({ item: element, type, filePath: currentPath })
    }
}

const searchRecursively = (searchTerm: string, element: any, type: string, currentPath: string) => {
    ...
    if ('members' in element && Array.isArray((element as any).members)) {
        ;(element as any).members.forEach((m: any) =>
            searchInElement(searchTerm, m as any, 'Member', `${currentPath}.${m.name}`),
        )
    }
}
