...
function getAttr(element: Element, attributeName: string, ns?: string): string | undefined {
    return ns
        ? (element.getAttributeNS(ns, attributeName) ?? undefined)
...
function parseParameters(element: Element, _pathPrefix: string): GirParameter[] {
    ...
    Array.from(paramContainer.children)
        .filter(
            (child) =>
                (child.localName === 'parameter' || child.localName === 'instance-parameter') &&
                child.namespaceURI === CORE_NS,
        )
        .forEach((pEl) => {
            const typeEl = Array.from(pEl.children).find(
                (child) =>
                    (child.localName === 'type' ||
                        child.localName === 'varargs' ||
...
function parseReturnValue(element: Element): GirReturnValue | undefined {
    ...
    const typeEl = Array.from(rvEl.children).find(
        (child) =>
            (child.localName === 'type' || child.localName === 'array') &&
            child.namespaceURI === CORE_NS,
    )
    return {
        doc: getDoc(rvEl),
...
function parseBaseElement(
    ...
    return {
        ...
        deprecated:
            getAttr(element, 'deprecated') ||
            (docDeprecatedEl ? docDeprecatedEl.textContent || true : false),
...
function parseCallable(
    ...
    pathPrefix: string,
    kind: GirElementKind,
): GirFunction | GirConstructor | GirMethod | GirCallback {
...
function parseField(element: Element, pathPrefix: string): GirField {
    ...
    const directType = childrenOfField.find(
        (c) => c.localName === 'type' && c.namespaceURI === CORE_NS,
    )
    if (directType) {
        typeEl = directType
    } else {
        const directArray = childrenOfField.find(
            (c) => c.localName === 'array' && c.namespaceURI === CORE_NS,
        )
        if (directArray) {
            typeEl = directArray // This could be an array of types, parser might need to handle
            // <type> inside <array>
        } else {
            const callbackEl = childrenOfField.find(
                (c) => c.localName === 'callback' && c.namespaceURI === CORE_NS,
            )
            if (callbackEl) {
                // A field that is a callback will have its type defined inside the <callback> element
                typeEl =
                    Array.from(callbackEl.children).find(
                        (c) => c.localName === 'type' && c.namespaceURI === CORE_NS,
                    ) || undefined
            }
        }
    ...
    return {
        ...
        type: typeEl
            ? getAttr(typeEl, 'name')
            : getAttr(element, 'name', C_NS) === 'gpointer'
              ? 'gpointer'
              : undefined,
...
function parseMembers(element: Element, _pathPrefix: string): GirMember[] {
    ...
    Array.from(element.children)
        .filter((child) => child.localName === 'member' && child.namespaceURI === CORE_NS)
        .forEach((mEl) => {
...
function parseBitfield(element: Element, pathPrefix: string): GirBitfield {
    const base = parseBaseElement(element, pathPrefix)
    const name = base.name!
    return {
        ...base,
        id: `${pathPrefix}.${name}`,
        kind: GirElementKind.Bitfield,
        cType: getAttr(element, 'type', C_NS),
        glibTypeName: getAttr(element, 'type-name', GLIB_NS),
        glibGetType: getAttr(element, 'get-type', GLIB_NS),
        members: parseMembers(element, `${pathPrefix}.${name}`),
...
function parseComplexType<T extends GirClass | GirInterface | GirRecord | GirUnion>(
    element: Element,
    pathPrefix: string,
    kind:
    ...
    children
        .filter((el) => el.localName === 'constructor' && el.namespaceURI === CORE_NS)
        .forEach((el) =>
            constructors.push(
                parseCallable(el, currentPath, GirElementKind.Constructor) as GirConstructor,
            ),
        )

    const methods: GirMethod[] = []
    children
        .filter((el) => el.localName === 'method' && el.namespaceURI === CORE_NS)
        .forEach((el) =>
            methods.push(parseCallable(el, currentPath, GirElementKind.Method) as GirMethod),
        )

    const functions: GirFunction[] = []
    children
        .filter((el) => el.localName === 'function' && el.namespaceURI === CORE_NS)
        .forEach((el) =>
            functions.push(parseCallable(el, currentPath, GirElementKind.Function) as GirFunction),
        )

    const properties: GirProperty[] = []
    children
        .filter((el) => el.localName === 'property' && el.namespaceURI === CORE_NS)
        .forEach((el) => properties.push(parseProperty(el, currentPath)))
    ...
    children
        .filter(
            (el) =>
                el.localName === 'signal' &&
                (el.namespaceURI === GLIB_NS || el.namespaceURI === CORE_NS),
        )
        .forEach((el) => signals.push(parseSignal(el, currentPath)))
    ...
    children
        .filter((el) => el.localName === 'field' && el.namespaceURI === CORE_NS)
        .forEach((el) => fields.push(parseField(el, currentPath)))
    //}

    const callbacks: GirCallback[] = []
    children
        .filter((el) => el.localName === 'callback' && el.namespaceURI === CORE_NS)
        .forEach((el) =>
            callbacks.push(parseCallable(el, currentPath, GirElementKind.Callback) as GirCallback),
        )

    const constants: GirConstant[] = []
    children
        .filter((el) => el.localName === 'constant' && el.namespaceURI === CORE_NS)
        .forEach((el) => constants.push(parseConstant(el, currentPath)))

    const records: GirRecord[] = []
    children
        .filter((el) => el.localName === 'record' && el.namespaceURI === CORE_NS)
        .forEach((el) => records.push(parseComplexType(el, currentPath, GirElementKind.Record)))

    const unions: GirUnion[] = []
    children
        .filter((el) => el.localName === 'union' && el.namespaceURI === CORE_NS)
        .forEach((el) => unions.push(parseComplexType(el, currentPath, GirElementKind.Union)))

    const prerequisites: string[] = []
    children
        .filter(
            (prereqEl) =>
                prereqEl.localName === 'prerequisite' && prereqEl.namespaceURI === CORE_NS,
        )
        .forEach((prereqEl) => {
    ...
    children
        .filter((implEl) => implEl.localName === 'implements' && implEl.namespaceURI === CORE_NS)
        .forEach((implEl) => {
...
export function parseGirContent(xmlString: string, fileName: string): GirRepository {
    ...
    if (!repoElement || repoElement.localName !== 'repository') {
        ...
        if (errorNode) {
            throw new Error(
                `Invalid GIR file: XML parsing error. Details: ${errorNode.textContent}`,
            )
        }
    ...
    repoChildren
        .filter((incEl) => incEl.localName === 'include' && incEl.namespaceURI === CORE_NS)
        .forEach((incEl) => {
            includes.push({
                name: getAttr(incEl, 'name')!,
                version: getAttr(incEl, 'version')!,
            })
        })

    const packages: string[] = []
    repoChildren
        .filter((pkgEl) => pkgEl.localName === 'package' && pkgEl.namespaceURI === CORE_NS)
        .forEach((pkgEl) => {
    ...
    repoChildren
        .filter((nsEl) => nsEl.localName === 'namespace' && nsEl.namespaceURI === CORE_NS)
        .forEach((nsEl) => {
            ...
            nsChildren
                .filter((el) => el.localName === 'class' && el.namespaceURI === CORE_NS)
                .forEach((el) =>
                    currentNamespace.classes.push(
                        parseComplexType(el, nsPath, GirElementKind.Class),
                    ),
                )
            nsChildren
                .filter((el) => el.localName === 'interface' && el.namespaceURI === CORE_NS)
                .forEach((el) =>
                    currentNamespace.interfaces.push(
                        parseComplexType(el, nsPath, GirElementKind.Interface),
                    ),
                )
            nsChildren
                .filter((el) => el.localName === 'record' && el.namespaceURI === CORE_NS)
                .forEach((el) =>
                    currentNamespace.records.push(
                        parseComplexType(el, nsPath, GirElementKind.Record),
                    ),
                )
            nsChildren
                .filter((el) => el.localName === 'union' && el.namespaceURI === CORE_NS)
                .forEach((el) =>
                    currentNamespace.unions.push(
                        parseComplexType(el, nsPath, GirElementKind.Union),
                    ),
                )
            nsChildren
                .filter((el) => el.localName === 'enumeration' && el.namespaceURI === CORE_NS)
                .forEach((el) => currentNamespace.enums.push(parseEnum(el, nsPath)))
            nsChildren
                .filter((el) => el.localName === 'bitfield' && el.namespaceURI === CORE_NS)
                .forEach((el) => currentNamespace.bitfields.push(parseBitfield(el, nsPath)))
            nsChildren
                .filter((el) => el.localName === 'alias' && el.namespaceURI === CORE_NS)
                .forEach((el) => currentNamespace.aliases.push(parseAlias(el, nsPath)))
            nsChildren
                .filter((el) => el.localName === 'constant' && el.namespaceURI === CORE_NS)
                .forEach((el) => currentNamespace.constants.push(parseConstant(el, nsPath)))
            nsChildren
                .filter((el) => el.localName === 'function' && el.namespaceURI === CORE_NS)
                .forEach((el) =>
                    currentNamespace.functions.push(
                        parseCallable(el, nsPath, GirElementKind.Function) as GirFunction,
                    ),
                )
            nsChildren
                .filter((el) => el.localName === 'callback' && el.namespaceURI === CORE_NS)
                .forEach((el) =>
                    currentNamespace.callbacks.push(
                        parseCallable(el, nsPath, GirElementKind.Callback) as GirCallback,
                    ),
                )
